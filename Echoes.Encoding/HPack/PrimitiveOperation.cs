    ...
    internal class PrimitiveOperation
    {
        ...
        public Span<char> ReadString(ReadOnlySpan<byte> input, Span<char> buffer, out int newOffset)
        {
            try
            {
                ...
                if (!huffmanEncoded)
                {
                    var res = System.Text.Encoding.ASCII.GetChars(rawString, buffer);
                ...
                var tempResult = System.Text.Encoding.ASCII.GetChars(decoded, buffer);

                return buffer.Slice(0, tempResult.Length);
        ...
        public Span<byte> WriteString(ReadOnlySpan<char> input, Span<byte> buffer, bool huffmanEncoded)
        {
            try
            {
                ...
                var inputBytes = System.Text.Encoding.ASCII.GetBytes(input, inputByteBuffer);
